...
int main()
{
    ...
    setupTimer(gcIntervalSeconds);
    startGarbageCollection();
    allocateNextMemBlock(200, &sMemBlock);
    allocateNextMemBlock(150, &sMemBlock);
    allocateNextMemBlock(310, &sMemBlock);
    allocateNextMemBlock(250, &sMemBlock);
    allocateNextMemBlock(170, &sMemBlock);
    allocateNextMemBlock(410, &sMemBlock);
    allocateNextMemBlock(280, &sMemBlock);
    allocateNextMemBlock(175, &sMemBlock);
    freeMemBlock(sMemBlock->next);
    freeMemBlock(sMemBlock->next->next->next);
    freeMemBlock(sMemBlock->next->next->next->next);
    freeMemBlock(sMemBlock->next->next->next->next->next->next);
    printMemBlocks();
    int *space = (int *)my_malloc(1, 5 * sizeof(int));
    int *ptr = space;
    for (int j = 0; j < 5; j++)
    {
        scanf("%d", space);
    ...
    printMemBlocks();
    char *ch = (char *)my_malloc(3, 6 * sizeof(char));
    char *c = ch;
    for (int j = 0; j < 5; j++)
    {
        while (getchar() != '\n')
    ...
    printMemBlocks();
    printMemBlocks();
    int *space1 = (int *)my_malloc(1, 4 * sizeof(int));
    int *ptr1 = space1;
    for (int j = 0; j < 4; j++)
    {
        scanf("%d", space1);
    ...
    printMemBlocks();
    char *c1 = (char *)my_calloc(3, 6, sizeof(char));
    char *c2 = c1;
    for (int j = 0; j < 5; j++)
    {
        while (getchar() != '\n')
